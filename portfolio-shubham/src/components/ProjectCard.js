...
export default function ProjectCard(props) {
  const handleOnClick = (liveLink) => {
    if (liveLink) window.open(`${props.deployedProjectLink}`);
    else window.open(`${props.projectLink}`);
  };
    ...
    <div className="project-window" id={props.id}>
      <div className={`project-wrapper ${props.className}`}>
        <div className="about-project">
          ...
          <button
            className="btn"
            onClick={() => handleOnClick(props.deployedProjectLink)}
          >
            <span>
              Live View
              {/* <FiGithub
